...
public class RCon {
	...
	public String[] banIPList() throws IOException, AuthenticationException {
		String[] data=Utils.lines(send("banlist ips"));
		if (data.length >= 2) {
			return data[1].split("\\, ");
		}
	...
	public String[] banList() throws IOException, AuthenticationException {
		String[] data=Utils.lines(send("banlist players"));
		if (data.length >= 2) {
			return data[1].split("\\, ");
		}
		return Constant.EMPTY_STRING_ARRAY;
	...
	public String[] list() throws IOException, AuthenticationException {
		String[] data=Utils.lines(send("list"));
		if (data.length >= 2) {
			return data[1].split("\\, ");
		}
		return Constant.EMPTY_STRING_ARRAY;
	...
	private String send(final int type, final String payload) throws IOException, IncorrectRequestIdException {
		final String responsePayload = new String(send(type, payload.getBytes(CompatCharsets.US_ASCII)),
				CompatCharsets.US_ASCII);
